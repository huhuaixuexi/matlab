...
fprintf('========================================\n');
fprintf('开始生成Magnos206氧分析仪9天模拟数据\n');
fprintf('每天前12小时正常 + 后12小时异常（数据保持）\n');
fprintf('========================================\n\n');
...
%% 新增数据保持功能
% 第10天：前半天正常 + 后半天数据保持
fprintf('[新增] 正在生成数据保持示例：前半天正常 + 后半天保持最后值...\n');
% 前半天正常数据
...
fprintf('[2/9] 正在生成第2天数据：前半天正常 + 后半天小幅度正向偏移...\n');
% 前半天正常数据
...
fprintf('[3/9] 正在生成第3天数据：前半天正常 + 后半天小幅度负向偏移...\n');
first_half = normal_mean + normal_std * randn(points_per_half, 1);
first_half = max(range_min, min(range_max, first_half));
...
fprintf('[4/9] 正在生成第4天数据：前半天正常 + 后半天大幅度正向偏移...\n');
first_half = normal_mean + normal_std * randn(points_per_half, 1);
first_half = max(range_min, min(range_max, first_half));
...
fprintf('[5/9] 正在生成第5天数据：前半天正常 + 后半天大幅度负向偏移...\n');
first_half = normal_mean + normal_std * randn(points_per_half, 1);
first_half = max(range_min, min(range_max, first_half));
...
fprintf('[6/9] 正在生成第6天数据：前半天正常 + 后半天剧烈波动...\n');
first_half = normal_mean + normal_std * randn(points_per_half, 1);
first_half = max(range_min, min(range_max, first_half));
...
fprintf('[7/9] 正在生成第7天数据：前半天正常 + 后半天满量程输出...\n');
first_half = normal_mean + normal_std * randn(points_per_half, 1);
first_half = max(range_min, min(range_max, first_half));
...
fprintf('[8/9] 正在生成第8天数据：前半天正常 + 后半天零位输出...\n');
first_half = normal_mean + normal_std * randn(points_per_half, 1);
first_half = max(range_min, min(range_max, first_half));
...
fprintf('[9/9] 正在生成第9天数据：前半天正常 + 后半天信号丢失...\n');
first_half = normal_mean + normal_std * randn(points_per_half, 1);
first_half = max(range_min, min(range_max, first_half));
...
sheet_names = {'数据保持示例', ...
               '第1天_全天正常', ...
               '第2天_小幅度正向偏移', '第3天_小幅度负向偏移', ...
               '第4天_大幅度正向偏移', '第5天_大幅度负向偏移', ...
               '第6天_剧烈波动异常', ...
               '第7天_满量程输出', '第8天_零位输出', '第9天_信号丢失'};
...
figure('Name', 'Magnos206数据预览（含数据保持示例）', 'Position', [50, 50, 1600, 1000]);
...
sgtitle('Magnos206氧分析仪 - 数据预览（红色虚线为12:00分界线）', ...
        'FontSize', 14, 'FontWeight', 'bold');

preview_filename = 'Magnos206_DataPreview_with_hold.png';
