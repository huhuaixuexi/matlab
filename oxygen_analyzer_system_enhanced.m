...
if config.enable_visualization
     ...
     % === 右侧维护操作记录区域 ===
     record_panel = uipanel('Parent', main_panel, ...
                           'Position', [0.51 0.01 0.48 0.24], ...
                           'BackgroundColor', colors.light, ...
                           'Title', '维护操作记录', ...
                           'TitlePosition', 'centertop', ...
                           'FontSize', 12, ...
     ...
     % 确保面板标题字体正确设置
     try
         set(record_panel, 'FontName', 'Microsoft YaHei');
     catch
         try
             set(record_panel, 'FontName', 'SimHei');
         catch
             set(record_panel, 'FontName', 'default');
         end
     end
     
     % 创建表格显示维护记录
     % 定义表格列名
     column_names = {'数据故障代码', '故障时间', '仪表故障代码', '故障描述/名称', ...
                    '故障类型', '严重程度', '优先级', '维修操作', '耗时', '工具'};
     
     % 初始化空数据 - 运行过程中保持空白
     empty_data = cell(0, 10);  % 创建空的0行10列表格
     
     % 创建表格 - 调整位置，字体下面空一栏
     h_maintenance_table = uitable('Parent', record_panel, ...
                                  'Position', [5 5 record_panel.Position(3)*figWidth-10 record_panel.Position(4)*figHeight-50], ...
                                  'Data', empty_data, ...
                                  'ColumnName', column_names, ...
                                  'ColumnWidth', {80, 120, 80, 120, 100, 80, 60, 150, 80, 100}, ...
                                  'FontSize', 8, ...
                                  'FontName', 'Microsoft YaHei', ...
                                  'BackgroundColor', colors.light, ...
                                  'ForegroundColor', colors.dark, ...
     ...
     % 添加说明文字
     uicontrol('Parent', record_panel, ...
              'Style', 'text', ...
              'String', '注：此表格用于工人师傅实际维护后手动填写记录', ...
              'Position', [10 record_panel.Position(4)*figHeight-45 record_panel.Position(3)*figWidth-20 20], ...
              'BackgroundColor', colors.light, ...
              'ForegroundColor', colors.dark, ...
              'FontSize', 9, ...
              'FontName', 'Microsoft YaHei', ...
     ...
     % 确保表格字体正确设置
     try
         set(h_maintenance_table, 'FontName', 'Microsoft YaHei');
     catch
         try
             set(h_maintenance_table, 'FontName', 'SimHei');
         catch
             set(h_maintenance_table, 'FontName', 'default');
         end
     end
     ...
     % 将表格句柄保存到应用数据中
     setappdata(fig, 'maintenance_table_handle', h_maintenance_table);
     setappdata(fig, 'maintenance_table_data', maintenance_table_data);
...
for i = 1:total_samples
     ...
     if is_alarm
         ...
         if should_alarm
             ...
             alarm_datetime(end+1) = current_time;  % 记录实际时间
             ...
             if config.show_maintenance_advice && isfield(maintenance_advice, 'all_suggestions') && ~isempty(maintenance_advice.all_suggestions)
                 ...
                 % 弹出维护操作记录窗口
                 if config.enable_visualization
                     show_maintenance_input_dialog(fault_code, current_time, maintenance_advice);
                 end
             end
         end
     end
...
function maintenance_table = create_maintenance_table(maintenance_records)
    ...
    % 创建表格数据
    maintenance_table = table(...
        maintenance_records.data_fault_code', ...
        maintenance_records.fault_time', ...
        maintenance_records.instrument_fault_code', ...
        maintenance_records.fault_description', ...
        maintenance_records.fault_type', ...
        maintenance_records.severity', ...
        priority_cell, ...
        maintenance_records.maintenance_action', ...
        maintenance_records.time_cost', ...
        maintenance_records.tools', ...
        'VariableNames', {...
            'DataFaultCode', ...
            'FaultTime', ...
            'InstrumentFaultCode', ...
    ...
    % 设置中文列名（如果支持的话）
    try
        maintenance_table.Properties.VariableNames = {...
            '数据故障代码', ...
            '故障时间', ...
            '仪表故障代码', ...
            '故障描述名称', ...
...
% 显示维护操作记录对话框函数
function show_maintenance_input_dialog(fault_code, fault_time, maintenance_advice)
    % 检查是否有维修建议
    if ~isfield(maintenance_advice, 'all_suggestions') || isempty(maintenance_advice.all_suggestions)
        return;
    end
    ...
    % 获取主窗口句柄（从工作区获取）
    main_fig = findobj('Name', '氧分析仪智能诊断维护系统 - 专业版');
    
    % 创建对话框
    dialog_fig = figure('Name', '维护操作记录', ...
                       'Position', [350, 150, 700, 600], ...
                       'NumberTitle', 'off', ...
                       'MenuBar', 'none', ...
                       'ToolBar', 'none', ...
                       'Resize', 'off', ...
    ...
    % 设置字体
    try
        set(dialog_fig, 'DefaultUicontrolFontName', 'Microsoft YaHei');
    catch
        set(dialog_fig, 'DefaultUicontrolFontName', 'SimHei');
    end
    
    % 标题
    uicontrol('Parent', dialog_fig, ...
             'Style', 'text', ...
             'String', sprintf('故障代码: %s - 发生时间: %s', fault_code, datestr(fault_time, 'yyyy-mm-dd HH:MM:SS')), ...
             'Position', [20 550 660 30], ...
             'FontSize', 14, ...
             'FontWeight', 'bold', ...
    ...
    for i = 1:length(maintenance_advice.all_suggestions)
        suggestion = maintenance_advice.all_suggestions{i};
        suggestion_text{end+1} = sprintf('【建议%d】%s - %s', i, suggestion.fault_code, suggestion.fault_name);
        ...
        for j = 1:length(measures)
            ...
            suggestion_text{end+1} = sprintf('  步骤%d: %s (耗时: %s, 工具: %s)', ...
                                            measure.step, measure.action, measure.time, measure.tools);
        end
        suggestion_text{end+1} = '';
    end
    
    uicontrol('Parent', dialog_fig, ...
             'Style', 'listbox', ...
             'String', suggestion_text, ...
             'Position', [20 350 660 150], ...
             'FontSize', 9, ...
             'Max', 2);
    
    % 实际操作输入区域 - 与主界面表格10列保持一致
    uicontrol('Parent', dialog_fig, ...
             'Style', 'text', ...
             'String', '请填写实际执行的维护操作（与主界面表格对应）:', ...
             'Position', [20 310 400 20], ...
             'FontSize', 12, ...
             'FontWeight', 'bold', ...
             'HorizontalAlignment', 'left');
    
    % 第一行：数据故障代码（自动填充）、故障时间（自动填充）
    uicontrol('Parent', dialog_fig, ...
             'Style', 'text', ...
             'String', '数据故障代码:', ...
    ...
    h_data_fault_code = uicontrol('Parent', dialog_fig, ...
                                 'Style', 'edit', ...
                                 'Position', [130 280 80 25], ...
                                 'String', fault_code, ...
                                 'Enable', 'off');
    ...
    uicontrol('Parent', dialog_fig, ...
             'Style', 'text', ...
             'String', '故障时间:', ...
             'Position', [230 280 80 20], ...
             'HorizontalAlignment', 'left');
    
    h_fault_time = uicontrol('Parent', dialog_fig, ...
                            'Style', 'edit', ...
                            'Position', [320 280 150 25], ...
                            'String', datestr(fault_time, 'yyyy-mm-dd HH:MM:SS'), ...
                            'Enable', 'off');
    
    % 第二行：仪表故障代码、故障描述/名称（从建议中选择）
    uicontrol('Parent', dialog_fig, ...
             'Style', 'text', ...
             'String', '仪表故障代码:', ...
    ...
    for i = 1:length(maintenance_advice.all_suggestions)
        suggestion = maintenance_advice.all_suggestions{i};
        instrument_codes{end+1} = suggestion.fault_code;
        fault_descriptions{end+1} = suggestion.fault_name;
        fault_types{end+1} = get_fault_type_name(fault_code);
        severities{end+1} = suggestion.severity;
        priorities{end+1} = num2str(suggestion.priority);
    end
    
    h_instrument_code = uicontrol('Parent', dialog_fig, ...
                                 'Style', 'popupmenu', ...
                                 'Position', [130 250 80 25], ...
                                 'String', instrument_codes, ...
                                 'Callback', @update_related_fields);
    
    uicontrol('Parent', dialog_fig, ...
             'Style', 'text', ...
             'String', '故障描述:', ...
             'Position', [230 250 80 20], ...
             'HorizontalAlignment', 'left');
    
    h_fault_desc = uicontrol('Parent', dialog_fig, ...
                            'Style', 'edit', ...
                            'Position', [320 250 200 25], ...
                            'String', fault_descriptions{1});
    ...
    h_fault_type = uicontrol('Parent', dialog_fig, ...
                            'Style', 'edit', ...
                            'Position', [110 220 120 25], ...
                            'String', fault_types{1}, ...
                            'Enable', 'off');
    
    uicontrol('Parent', dialog_fig, ...
             'Style', 'text', ...
             'String', '严重程度:', ...
             'Position', [250 220 80 20], ...
             'HorizontalAlignment', 'left');
    
    h_severity = uicontrol('Parent', dialog_fig, ...
                          'Style', 'edit', ...
                          'Position', [340 220 80 25], ...
                          'String', severities{1}, ...
                          'Enable', 'off');
    ...
    uicontrol('Parent', dialog_fig, ...
             'Style', 'text', ...
             'String', '优先级:', ...
             'Position', [440 220 60 20], ...
             'HorizontalAlignment', 'left');
    
    h_priority = uicontrol('Parent', dialog_fig, ...
                          'Style', 'edit', ...
    ...
    h_maintenance_action = uicontrol('Parent', dialog_fig, ...
                                    'Style', 'edit', ...
                                    'Position', [20 120 660 50], ...
                                    'Max', 3, ...
                                    'Min', 1);
    ...
    h_actual_time = uicontrol('Parent', dialog_fig, ...
                             'Style', 'edit', ...
                             'Position', [110 90 120 25]);
    ...
    h_tools = uicontrol('Parent', dialog_fig, ...
                       'Style', 'edit', ...
                       'Position', [340 90 200 25]);
    ...
    uicontrol('Parent', dialog_fig, ...
             ...
             'FontSize', 12, ...
             'Callback', @(src,evt) record_and_close());
    
    uicontrol('Parent', dialog_fig, ...
             'Style', 'pushbutton', ...
             'String', '稍后填写', ...
             'Position', [370 20 100 35], ...
             'FontSize', 12, ...
             'Callback', @(src,evt) close(dialog_fig));
    
    % 更新相关字段函数
    function update_related_fields(src, ~)
        selected_idx = get(src, 'Value');
        set(h_fault_desc, 'String', fault_descriptions{selected_idx});
        set(h_fault_type, 'String', fault_types{selected_idx});
        set(h_severity, 'String', severities{selected_idx});
        set(h_priority, 'String', priorities{selected_idx});
    end
    
    % 记录并关闭函数
    function record_and_close()
        % 获取所有输入的信息
        data_fault_code = get(h_data_fault_code, 'String');
        fault_time_str = get(h_fault_time, 'String');
        instrument_code = instrument_codes{get(h_instrument_code, 'Value')};
        fault_desc = get(h_fault_desc, 'String');
        fault_type = get(h_fault_type, 'String');
        severity = get(h_severity, 'String');
        priority = get(h_priority, 'String');
        maintenance_action = get(h_maintenance_action, 'String');
        actual_time = get(h_actual_time, 'String');
        tools = get(h_tools, 'String');
        
        % 验证必填字段
        if isempty(strtrim(maintenance_action)) || isempty(strtrim(actual_time)) || isempty(strtrim(tools))
            msgbox('请填写完整的维护操作信息（维修操作、耗时、工具为必填项）', '输入提示', 'warn');
            return;
        end
        
        % 创建新的表格行数据
        new_row = {data_fault_code, fault_time_str, instrument_code, fault_desc, ...
                  fault_type, severity, priority, maintenance_action, actual_time, tools};
        
        % 更新主界面表格
        if ~isempty(main_fig)
            update_main_table(main_fig, new_row);
        end
        ...
        msgbox('维护操作记录已保存到主界面表格！', '记录成功', 'help');
        close(dialog_fig);
    end
end

% 更新主界面表格函数
function update_main_table(main_fig, new_row)
    % 获取表格句柄和当前数据
    table_handle = getappdata(main_fig, 'maintenance_table_handle');
    current_data = get(table_handle, 'Data');
    ...
    % 更新表格显示
    set(table_handle, 'Data', updated_data);
end      