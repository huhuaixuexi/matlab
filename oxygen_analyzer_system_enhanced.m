...
function show_maintenance_input_dialog(fault_code, fault_time, maintenance_advice)
    ...
    h_operator = uicontrol('Parent', dialog_fig, ...
                          'Style', 'edit', ...
                          'Position', [110 180 150 25]);
    ...
    h_action = uicontrol('Parent', dialog_fig, ...
                        'Style', 'edit', ...
                        'Position', [20 80 560 50], ...
                        'Max', 3, ...
                        'Min', 1);
    ...
    uicontrol('Parent', dialog_fig, ...
             'Style', 'text', ...
             'String', '实际耗时:', ...
             'Position', [300 180 80 20], ...
             'HorizontalAlignment', 'left');
    
    h_time = uicontrol('Parent', dialog_fig, ...
                      'Style', 'edit', ...
                      'Position', [390 180 100 25]);
    ...
    function record_and_close()
        ...
        if ~isempty(strtrim(operator_name)) && ~isempty(strtrim(actual_action)) && ~isempty(strtrim(actual_time))
            fprintf('\n【维护操作记录】\n');
            fprintf('故障代码: %s\n', fault_code);
            fprintf('故障时间: %s\n', datestr(fault_time, 'yyyy-mm-dd HH:MM:SS'));
            fprintf('操作员: %s\n', operator_name);
            fprintf('实际操作: %s\n', actual_action);
            fprintf('实际耗时: %s\n', actual_time);
            fprintf('记录时间: %s\n\n', datestr(now, 'yyyy-mm-dd HH:MM:SS'));
            
            msgbox('维护操作记录已保存！', '记录成功', 'help');
        else
            msgbox('请填写完整的维护操作信息', '输入提示', 'warn');
            return;
        end
