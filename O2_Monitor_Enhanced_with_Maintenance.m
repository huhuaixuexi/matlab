...
fprintf('  维护建议: %s\n', config.enable_maintenance ? '启用' : '禁用');
fprintf('\n');
...
if config.enable_visualization
     fig = figure('Name', '实时氧分析仪监控系统 v4.0 (集成维护建议)', ...
...
% 获取维护建议函数
function maintenance_msg = getMaintenanceRecommendation(fault_code, maintenanceDB)
     maintenance_msg = '';
     
     if isempty(maintenanceDB)
         return;
     end
...
for i = 1:total_samples
     ...
     if is_alarm
         ...
         if should_alarm
             ...
             % 获取维护建议
             if config.enable_maintenance
                 maintenance_msg = getMaintenanceRecommendation(fault_code, maintenanceDB);
...
if config.save_log
     % 生成报告文件名
     report_filename = sprintf('O2_Monitor_Enhanced_Maintenance_Report_%s.txt', ...
     ...
     if stats.total_alarms > 0
         alarm_excel = sprintf('O2_Alarms_Enhanced_Maintenance_%s.xlsx', datestr(now, 'yyyymmdd_HHMMSS'));
     ...
     if config.enable_maintenance && ~isempty(stats.maintenance_recommendations)
         ...
         for i = 1:length(stats.maintenance_recommendations)
             ...
             % 获取详细维护措施
             fault_info = maintenanceDB.getFaultByCode(rec.fault_code);
...
% 保存最终图形
if config.enable_visualization
     fig_filename = sprintf('O2_Monitor_Enhanced_Maintenance_Final_%s.png', ...
                          datestr(now, 'yyyymmdd_HHMMSS'));
