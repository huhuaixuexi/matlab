%% 故障诊断维修记录生成系统 - MATLAB版本
% 生成5年的模拟故障维修记录数据并导出为Excel文件
% 包含数据故障代码映射

function generate_fault_records_only()
    % 主函数
    fprintf('========================================\n');
    fprintf('故障诊断维修记录生成系统\n');
...
function generate_statistics(records)
    ...
    for i = 1:length(unique_types)
        ...
        percentage = count / size(records, 1) * 100;
        fprintf('  %s: %d (%.1f%%)\n', unique_types{i}, count, percentage);
    end
    ...
    for i = 1:length(unique_severities)
        ...
        percentage = count / size(records, 1) * 100;
        fprintf('  %s: %d (%.1f%%)\n', unique_severities{i}, count, percentage);
    end
    ...
    for i = 1:length(unique_data_faults)
        ...
        percentage = count / size(records, 1) * 100;
        fprintf('  %s: %d (%.1f%%)\n', unique_data_faults{i}, count, percentage);
    end
