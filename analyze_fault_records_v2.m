...
function analyze_fault_records_v2(excel_file)
    ...
    % 8. 生成预测性维护界面（第4页）
    try
        generate_predictive_maintenance_view(data, predictive_stats);
    catch ME
        fprintf('生成预测性维护界面失败: %s\n', ME.message);
    end
    
    % 9. 导出预测性维护结果到Excel
    try
        export_predictive_maintenance_to_excel(data, predictive_stats, excel_file);
    catch ME
        fprintf('导出预测性维护结果失败: %s\n', ME.message);
    end
...
% ========== 新增：第4页 预测性维护界面 ==========
function generate_predictive_maintenance_view(data, predictive_stats)
    fig4 = figure('Name', '故障分析综合报告 - 第4页（预测性维护）', ...
        'Position', [200, 50, 1400, 900], ...
        'Color', 'white');

    set(fig4, 'DefaultAxesFontSize', 11);
    set(fig4, 'DefaultTextFontSize', 12);

    % 1. 未来6个月故障预测
    subplot(2, 2, 1);
    monthly_data = predictive_stats.monthly_faults;
    future_months_dt = monthly_data.months(end) + calmonths(1:6);
    preds = max(0, round(predictive_stats.predictions(:)'));
    future_labels = cellstr(datestr(future_months_dt, 'yyyy-mm'));
    bar(categorical(future_labels), preds, 'FaceColor', [0.4 0.7 0.9]);
    title('未来6个月故障预测', 'FontSize', 14, 'FontWeight', 'bold');
    xlabel('月份', 'FontSize', 12, 'FontWeight', 'bold');
    ylabel('预测故障次数', 'FontSize', 12, 'FontWeight', 'bold');
    grid on;
    for i = 1:length(preds)
        text(i, preds(i), num2str(preds(i)), 'HorizontalAlignment', 'center', ...
            'VerticalAlignment', 'bottom', 'FontSize', 10);
    end

    % 2. 备件建议库存
    subplot(2, 2, 2);
    parts = predictive_stats.parts_freq;
    years_cnt = max(1, length(unique(data.year)));
    part_types_cn = {'传感器','电路板','模块','电缆'};
    part_counts = [parts.sensor, parts.board, parts.module, parts.cable];
    annual_need = part_counts / years_cnt;
    suggested_stock = ceil(annual_need * 1.5);
    bar(suggested_stock, 'FaceColor', [0.6 0.85 0.6]);
    set(gca, 'XTick', 1:4, 'XTickLabel', part_types_cn);
    title('建议库存（基于年均需求×1.5）', 'FontSize', 14, 'FontWeight', 'bold');
    xlabel('备件类型', 'FontSize', 12, 'FontWeight', 'bold');
    ylabel('建议库存（件）', 'FontSize', 12, 'FontWeight', 'bold');
    grid on;
    for i = 1:length(suggested_stock)
        text(i, suggested_stock(i), num2str(suggested_stock(i)), 'HorizontalAlignment', 'center', ...
            'VerticalAlignment', 'bottom', 'FontSize', 10);
    end
    ...
    [unique_codes, ~, idx] = unique(data.instrument_fault_code);
    code_counts = accumarray(idx, 1);
    [sorted_counts, order_idx] = sort(code_counts, 'descend');
    ...
    max_list = min(10, length(sorted_counts));
    for i = 1:max_list
        if sorted_counts(i) > 1
            code = unique_codes{order_idx(i)};
            code_mask = strcmp(data.instrument_fault_code, code);
            fault_dates = sort(data.fault_time(code_mask));
            if length(fault_dates) > 1
                intervals = days(diff(fault_dates));
                avg_interval = mean(intervals);
                sugg_interval = max(1, round(avg_interval * 0.8));
                name_idx = find(code_mask, 1);
                name = data.fault_name{name_idx};
                top_codes{end+1} = code; %#ok<AGROW>
                top_names{end+1} = name; %#ok<AGROW>
                avg_intervals(end+1) = avg_interval; %#ok<AGROW>
                sugg_intervals(end+1) = sugg_interval; %#ok<AGROW>
                occur_counts(end+1) = sum(code_mask); %#ok<AGROW>
            end
        end
    end
    
    if isempty(top_codes)
        text(0.5, 0.5, '暂无可计算的维护周期建议（样本不足）', 'HorizontalAlignment', 'center');
        axis off;
    else
        barh(sugg_intervals, 'FaceColor', [0.9 0.7 0.5]);
        set(gca, 'YTick', 1:length(top_codes));
        set(gca, 'YTickLabel', top_codes);
        title('维护周期建议（天）', 'FontSize', 14, 'FontWeight', 'bold');
        xlabel('建议维护周期（天）', 'FontSize', 12, 'FontWeight', 'bold');
        ylabel('故障代码', 'FontSize', 12, 'FontWeight', 'bold');
        grid on;
        for i = 1:length(sugg_intervals)
            text(sugg_intervals(i)+0.1, i, sprintf('%d天', sugg_intervals(i)), 'FontSize', 10);
        end
    end
    ...
    subplot(2, 2, 4);
    axis off;
    x0 = 0.02; y0 = 0.9; dy = 0.08;
    header = {'故障代码','故障名称','历史次数','平均间隔(天)','建议周期(天)'};
    txt = sprintf('%-12s %-18s %-8s %-12s %-12s', header{:});
    text(x0, y0, txt, 'Units', 'normalized', 'FontName', 'Monospaced', 'FontWeight', 'bold');
    
    for i = 1:min(8, length(top_codes))
        row = {top_codes{i}, truncate_text(top_names{i}, 9), num2str(occur_counts(i)), ...
               sprintf('%.0f', avg_intervals(i)), sprintf('%d', sugg_intervals(i))};
        line = sprintf('%-12s %-18s %-8s %-12s %-12s', row{:});
        text(x0, y0 - i*dy, line, 'Units', 'normalized', 'FontName', 'Monospaced');
    end

    sgtitle('故障诊断维修记录分析报告 - 预测性维护', 'FontSize', 16, 'FontWeight', 'bold');
end

% ========== 新增：导出预测性维护结果到Excel ==========
function export_predictive_maintenance_to_excel(data, predictive_stats, source_excel)
    [~, base, ~] = fileparts(source_excel);
    out_filename = sprintf('预测性维护结果_%s_%s.xlsx', base, datestr(now, 'yyyymmdd_HHMMSS'));

    % Sheet 1: 未来趋势
    monthly_data = predictive_stats.monthly_faults;
    future_months_dt = monthly_data.months(end) + calmonths(1:6);
    preds = max(0, round(predictive_stats.predictions(:)'));
    sheet1 = [{'月份','预测故障次数'}; [cellstr(datestr(future_months_dt,'yyyy-mm')), num2cell(preds')]];
    write_cell_compat(out_filename, '未来趋势', sheet1);
    ...
    annual_need = part_counts / years_cnt;
    suggested_stock = ceil(annual_need * 1.5);
    sheet2 = [{'备件类型','历史更换次数','年均需求','建议库存'}; ...
              [part_types_cn', num2cell(part_counts'), num2cell(annual_need'), num2cell(suggested_stock')]];
    write_cell_compat(out_filename, '备件需求', sheet2);
    ...
    rows = {};
    max_list = min(20, length(sorted_counts));
    for i = 1:max_list
        if sorted_counts(i) > 1
            code = unique_codes{order_idx(i)};
            code_mask = strcmp(data.instrument_fault_code, code);
            ...
            if length(fault_dates) > 1
                ...
                name_idx = find(code_mask, 1);
                name = data.fault_name{name_idx};
                rows(end+1, :) = {code, name, sum(code_mask), round(avg_interval), sugg_interval}; %#ok<AGROW>
            end
        end
    end
    if isempty(rows)
        rows = {'-', '样本不足，无法计算', 0, 0, 0};
    end
    sheet3 = [{'故障代码','故障名称','历史次数','平均间隔(天)','建议周期(天)'}; rows];
    write_cell_compat(out_filename, '维护周期', sheet3);

    fprintf('预测性维护结果已导出: %s\n', out_filename);
end

% 兼容写入单元格数据
function write_cell_compat(filename, sheet, cell_data)
    try
        writecell(cell_data, filename, 'Sheet', sheet);
    catch
        try
            xlswrite(filename, cell_data, sheet);
        catch ME
            warning('写入Excel失败: %s', ME.message);
        end
    end
end

% 文本截断（用于图中表格显示）
function s = truncate_text(s, maxChars)
    if numel(s) > maxChars
        s = [s(1:maxChars-1) '…']; %#ok<AGROW>
    end
end
