...
if config.enable_visualization
     fig = figure('Name', '氧分析仪智能诊断维护系统 v5.0', ...
...
for i = 1:total_samples
     ...
     if is_alarm
         ...
         if should_alarm
             ...
             % 显示维修建议
             if config.show_maintenance_advice && ~isempty(maintenance_advice.measures)
     ...
     if sample_count >= config.diagnosis_interval && ...
        mod(sample_count, config.diagnosis_interval) == 0
         ...
         % 显示维修建议
         if config.show_maintenance_advice && ~strcmp(fault_code, 'NORMAL') && ~isempty(maintenance_advice.measures)
             advice_text = format_maintenance_advice(maintenance_advice);
             fprintf('%s\n', advice_text);
...
if config.save_log
     % 生成报告文件名
     report_filename = sprintf('O2_Complete_System_Report_%s.txt', ...
     ...
     fprintf(fid, '\n\n【维修数据库概览】\n');
     fprintf(fid, '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
     fprintf(fid, '故障代码 | 故障名称 | 严重程度 | 优先级\n');
     fprintf(fid, '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
     
     db_fields = fieldnames(maintenance_db);
     for k = 1:length(db_fields)
         field = db_fields{k};
         info = maintenance_db.(field);
         fprintf(fid, '%-8s | %-30s | %-6s | %d\n', ...
                 info.fault_code, info.fault_name, info.severity, info.priority);
     end
     ...
     if stats.total_alarms > 0
         alarm_excel = sprintf('O2_Complete_System_Alarms_%s.xlsx', datestr(now, 'yyyymmdd_HHMMSS'));
...
% 保存最终图形
if config.enable_visualization
     fig_filename = sprintf('O2_Complete_System_Figure_%s.png', ...
                          datestr(now, 'yyyymmdd_HHMMSS'));
...
function maintenance_advice = get_maintenance_advice(fault_code, fault_mapping, maintenance_db)
    ...
    maintenance_advice.measures = {};
    maintenance_advice.severity = '';
    maintenance_advice.priority = 999;
    ...
    % 选择第一个匹配的维修建议（实际应用中可能需要更复杂的逻辑）
    for i = 1:length(possible_codes)
        maint_code = possible_codes{i};
        if isfield(maintenance_db, ['code_' strrep(maint_code, '-', '_')])
            maint_info = maintenance_db.(['code_' strrep(maint_code, '-', '_')]);
            maintenance_advice.fault_code = maint_code;
            maintenance_advice.fault_name = maint_info.fault_name;
            maintenance_advice.measures = maint_info.maintenance_measures;
            maintenance_advice.severity = maint_info.severity;
            maintenance_advice.priority = maint_info.priority;
            break;
        end
    end
end
