...
%% ========================================
%% 氧分析仪智能诊断维护系统 - 美化版 + 维修记录层
%% ========================================

fprintf('════════════════════════════════════════\n');
fprintf('   氧分析仪智能诊断维护系统 v7.1\n');
fprintf('      专业美化界面版 + 维修记录层\n');
fprintf('════════════════════════════════════════\n\n');
...
config.export_actual_records_on_finish = true;      % 程序结束是否自动导出“故障诊断维修记录.xlsx”
...
if width(raw_data) == 2
     % 两列数据：时间戳和氧浓度
     time_stamps = raw_data{:,1}; %#ok<NASGU>
     oxygen_values = raw_data{:,2};
elseif width(raw_data) == 1
     % 单列数据：仅氧浓度，自动生成时间戳
     oxygen_values = raw_data{:,1};
     time_stamps = (0:length(oxygen_values)-1)' * config.sample_interval; %#ok<NASGU>
else
     % 多列数据，假设第二列是氧浓度
     oxygen_values = raw_data{:,2};
     time_stamps = (0:length(oxygen_values)-1)' * config.sample_interval; %#ok<NASGU>
...
% 初始化记录层状态
records = struct('dataFaultCode', {}, 'faultTime', {}, 'instrumentFaultCode', {}, 'faultName', {}, 'faultType', {}, 'severity', {}, 'priority', {}, 'operation', {}, 'duration', {}, 'tools', {});
current_event_context = struct('dataFaultCode','', 'faultTime', NaT); % 用于预填
...
if config.enable_visualization
     ...
     buffer_panel = uipanel('Parent', main_panel, ...
                           ...
                           'FontName', 'Microsoft YaHei', ...
                           'ForegroundColor', colors.primary);
     safe_set_font(buffer_panel);
     
     ax2 = axes('Parent', buffer_panel, 'Position', [0.1 0.15 0.85 0.75]); %#ok<NASGU>
     ...
     h_log_text = uicontrol('Parent', log_panel, ...
                           ...
                           'Value', 1, ... % 初始选中第一行
                           'ListboxTop', 1); % 确保从第一行开始显示
     safe_set_font(h_log_text);

     % === 底部Tab：维修建议 / 维修记录 ===
     maint_tabgroup = uitabgroup('Parent', main_panel, ...
                                 'Units','normalized', ...
                                 'Position', [0.01 0.01 0.98 0.24]);
     tab_advice  = uitab(maint_tabgroup, 'Title', '维修建议');
     tab_records = uitab(maint_tabgroup, 'Title', '维修记录');

     % 建议页内容
     h_maintenance_text = uicontrol('Parent', tab_advice, ...
                                   'Style', 'listbox', ...
                                   'Units','normalized', ...
                                   ...
                                   'Value', 1, ... % 初始选中第一行
                                   'ListboxTop', 1); % 确保从第一行开始显示
     safe_set_font(h_maintenance_text);

     % 记录页左侧表单
     label_w = 0.16; input_w = 0.18; row_h = 0.12; left_x = 0.02; gap_x = 0.01; col2_x = left_x + label_w + input_w + gap_x; 
     
     make_label(tab_records, left_x, 0.82, label_w, row_h, '数据故障代码:', colors);
     h_rec_data_code  = make_edit(tab_records, left_x+label_w, 0.82, input_w, row_h, colors, true);

     make_label(tab_records, left_x, 0.67, label_w, row_h, '故障时间:', colors);
     h_rec_time       = make_edit(tab_records, left_x+label_w, 0.67, input_w, row_h, colors, true);

     make_label(tab_records, left_x, 0.52, label_w, row_h, '仪表故障代码(建议):', colors);
     h_rec_instr_popup = uicontrol('Parent', tab_records, 'Style','popupmenu', 'Units','normalized', ...
                            'Position', [left_x+label_w, 0.52, input_w, row_h], 'String', {'(无建议代码)'}, ...
                            'BackgroundColor', colors.light, 'ForegroundColor', colors.dark, 'FontSize', 9, 'FontName','Microsoft YaHei');
     safe_set_font(h_rec_instr_popup);

     make_label(tab_records, left_x, 0.37, label_w, row_h, '仪表故障代码(实际):', colors);
     h_rec_instr_code = make_edit(tab_records, left_x+label_w, 0.37, input_w, row_h, colors, false);

     make_label(tab_records, left_x, 0.22, label_w, row_h, '故障描述/名称:', colors);
     h_rec_name       = make_edit(tab_records, left_x+label_w, 0.22, input_w, row_h, colors, false);

     make_label(tab_records, col2_x, 0.82, label_w, row_h, '故障类型:', colors);
     h_rec_type       = make_edit(tab_records, col2_x+label_w, 0.82, input_w, row_h, colors, true);

     make_label(tab_records, col2_x, 0.67, label_w, row_h, '严重程度:', colors);
     h_rec_severity   = make_edit(tab_records, col2_x+label_w, 0.67, input_w, row_h, colors, false);

     make_label(tab_records, col2_x, 0.52, label_w, row_h, '优先级:', colors);
     h_rec_priority   = make_edit(tab_records, col2_x+label_w, 0.52, input_w, row_h, colors, false);

     make_label(tab_records, col2_x, 0.37, label_w, row_h, '维修操作:', colors);
     h_rec_op         = make_edit(tab_records, col2_x+label_w, 0.37, input_w, row_h, colors, false);

     make_label(tab_records, col2_x, 0.22, label_w, row_h, '耗时:', colors);
     h_rec_duration   = make_edit(tab_records, col2_x+label_w, 0.22, input_w, row_h, colors, false);

     make_label(tab_records, left_x, 0.07, label_w, row_h, '工具:', colors);
     h_rec_tools      = make_edit(tab_records, left_x+label_w, 0.07, input_w, row_h, colors, false);

     % 记录页右侧表格
     h_records_table = uitable('Parent', tab_records, 'Units','normalized', 'Position', [0.42 0.05 0.56 0.9], ...
                               'BackgroundColor', colors.light, 'ForegroundColor', colors.dark, 'FontSize', 9, 'FontName', 'Microsoft YaHei', ...
                               'ColumnName', {'数据故障代码','故障时间','仪表故障代码','故障描述/名称','故障类型','严重程度','优先级','维修操作','耗时','工具'}, ...
                               'Data', {});
     safe_set_font(h_records_table);

     % 按钮
     h_btn_add    = uicontrol('Parent', tab_records, 'Style','pushbutton', 'Units','normalized', ...
                              'Position', [0.74 0.93 0.10 0.06], 'String','添加记录', ...
                              'BackgroundColor', colors.secondary, 'ForegroundColor', colors.light, ...
                              'FontWeight','bold', 'FontSize', 10, 'FontName','Microsoft YaHei');
     safe_set_font(h_btn_add);

     h_btn_export = uicontrol('Parent', tab_records, 'Style','pushbutton', 'Units','normalized', ...
                              'Position', [0.86 0.93 0.12 0.06], 'String','导出Excel', ...
                              'BackgroundColor', colors.primary, 'ForegroundColor', colors.light, ...
                              'FontWeight','bold', 'FontSize', 10, 'FontName','Microsoft YaHei');
     safe_set_font(h_btn_export);

     % 回调与状态注册
     state = struct();
     state.records = records;
     state.h_records_table   = h_records_table;
     state.h_data_code       = h_rec_data_code;
     state.h_time            = h_rec_time;
     state.h_instr_popup     = h_rec_instr_popup;
     state.h_instr_code      = h_rec_instr_code;
     state.h_name            = h_rec_name;
     state.h_type            = h_rec_type;
     state.h_severity        = h_rec_severity;
     state.h_priority        = h_rec_priority;
     state.h_operation       = h_rec_op;
     state.h_duration        = h_rec_duration;
     state.h_tools           = h_rec_tools;
     state.maintenance_db    = maintenance_db;
     state.current_event     = current_event_context;

     setappdata(fig, 'record_state', state);

     % 绑定回调
     h_rec_instr_popup.Callback = @(src,evt) onInstrPopupChanged(fig);
     h_btn_add.Callback         = @(src,evt) onAddRecord(fig);
     h_btn_export.Callback      = @(src,evt) onExportRecords(fig);
...
for i = 1:total_samples
     ...
     if is_alarm
         ...
         if should_alarm
             ...
             % 预填记录层（基于本次报警事件）
             if config.enable_visualization
                 prefill_record_from_event(fig, fault_code, current_time, maintenance_advice, maintenance_db);
             end
         end
     end
     ...
     if sample_count >= config.diagnosis_interval && ...
        mod(sample_count, config.diagnosis_interval) == 0
         ...
         % 执行诊断
         [fault_code_pd, diagnosis_msg, maintenance_advice_pd] = periodic_diagnosis_enhanced(...
         ...
         % 统计定期诊断的故障（非正常状态）
         if ~strcmp(fault_code_pd, 'NORMAL')
             stats.total_alarms = stats.total_alarms + 1;
             if isKey(stats.fault_counts, fault_code_pd)
                 stats.fault_counts(fault_code_pd) = stats.fault_counts(fault_code_pd) + 1;
             else
                 stats.fault_counts(fault_code_pd) = 1;
             end
         end
         
         % 显示维修建议
         if config.show_maintenance_advice && ~strcmp(fault_code_pd, 'NORMAL') && isfield(maintenance_advice_pd, 'all_suggestions') && ~isempty(maintenance_advice_pd.all_suggestions)
             advice_text = format_maintenance_advice(maintenance_advice_pd);
             fprintf('%s\n', advice_text);
             maintenance_log{end+1} = [diagnosis_msg advice_text]; %#ok<SAGROW>
             current_maintenance_advice = maintenance_advice_pd;
         end

         % 预填记录层（基于本次定期诊断）
         if config.enable_visualization && ~strcmp(fault_code_pd, 'NORMAL')
             prefill_record_from_event(fig, fault_code_pd, current_time, maintenance_advice_pd, maintenance_db);
         end
...
% 导出维修记录（实际维护措施）到Excel
if config.enable_visualization && config.export_actual_records_on_finish
    try
        onExportRecords(fig);
    catch ME
        warning('导出"故障诊断维修记录.xlsx"失败: %s', ME.message);
    end
end
...
fprintf('✓ 完整的故障统计分析\n');
fprintf('✓ 可视化监控界面\n');
fprintf('✓ 维修记录层（实际维护措施记录 + 导出Excel）\n');
...
function maintenance_advice = get_maintenance_advice(fault_code, fault_mapping, maintenance_db)
    ...
    for i = 1:length(possible_codes)
        ...
        if isfield(maintenance_db, field_name)
            ...
            if isfield(maint_info, 'fault_type')
                single_advice.fault_type = maint_info.fault_type;
            else
                single_advice.fault_type = '';
            end
...
% 安全设置字体（含中文回退）
function safe_set_font(h)
    try
        set(h, 'FontName', 'Microsoft YaHei');
    catch
        try
            set(h, 'FontName', 'SimHei');
        catch
            try
                set(h, 'FontName', 'SimSun');
            catch
...
% 简化创建标签
function h = make_label(parent, x, y, w, hgt, textStr, colors)
    h = uicontrol('Parent', parent, 'Style','text', 'Units','normalized', ...
                  'Position', [x y w hgt], 'String', textStr, ...
                  'BackgroundColor', colors.light, 'ForegroundColor', [0.1 0.1 0.1], ...
                  'HorizontalAlignment','right', 'FontWeight','bold', 'FontSize', 9, 'FontName','Microsoft YaHei');
    safe_set_font(h);
end

% 简化创建输入框（editable可控）
function h = make_edit(parent, x, y, w, hgt, colors, readonly)
    style = 'edit';
    h = uicontrol('Parent', parent, 'Style', style, 'Units','normalized', ...
                  'Position', [x y w hgt], 'BackgroundColor', [1 1 1], 'ForegroundColor', [0 0 0], ...
                  'HorizontalAlignment','left', 'FontSize', 9, 'FontName', 'Microsoft YaHei');
    if readonly
...
% 根据事件预填维修记录区域
function prefill_record_from_event(fig, data_fault_code, fault_time, maintenance_advice, maintenance_db)
    if ~ishandle(fig) || ~isappdata(fig, 'record_state'); return; end
    state = getappdata(fig, 'record_state');

    % 预填诊断侧的信息
    set(state.h_data_code, 'String', data_fault_code);
    set(state.h_time, 'String', datestr(fault_time, 'yyyy-mm-dd HH:MM:SS'));

    % 生成建议下拉与默认显示
    popup_items = {'(无建议代码)'};
    if isfield(maintenance_advice,'all_suggestions') && ~isempty(maintenance_advice.all_suggestions)
        n = length(maintenance_advice.all_suggestions);
        popup_items = cell(1,n);
        for k = 1:n
            s = maintenance_advice.all_suggestions{k};
            popup_items{k} = sprintf('%s - %s', s.fault_code, s.fault_name);
        end
        set(state.h_instr_popup, 'String', popup_items, 'Value', 1);
        % 用首条建议预填
        s1 = maintenance_advice.all_suggestions{1};
        set(state.h_instr_code, 'String', s1.fault_code);
        set(state.h_name, 'String', s1.fault_name);
        % 类型从DB取（更权威）
        fn = ['code_' strrep(s1.fault_code,'-','_')];
        if isfield(maintenance_db, fn) && isfield(maintenance_db.(fn), 'fault_type')
            set(state.h_type, 'String', maintenance_db.(fn).fault_type);
        else
            if isfield(s1,'fault_type'); set(state.h_type,'String', s1.fault_type); else; set(state.h_type,'String',''); end
        end
        set(state.h_severity, 'String', s1.severity);
        set(state.h_priority, 'String', num2str(s1.priority));
    else
        set(state.h_instr_popup, 'String', popup_items, 'Value', 1);
        set(state.h_instr_code, 'String', '');
        % 无建议时，尝试以数据故障代码映射名称
        [dname, dtype] = data_fault_name_type(data_fault_code);
        set(state.h_name, 'String', dname);
        set(state.h_type, 'String', dtype);
        set(state.h_severity, 'String', '');
        set(state.h_priority, 'String', '');
    end

    % 清空由人工填写的实际措施字段
    set(state.h_operation, 'String', '');
    set(state.h_duration, 'String', '');
    set(state.h_tools, 'String', '');

    % 更新上下文
    state.current_event.dataFaultCode = data_fault_code;
    state.current_event.faultTime     = fault_time;
    setappdata(fig, 'record_state', state);
end

% 下拉选择变化：同步到实际输入框与相关字段
function onInstrPopupChanged(fig)
    if ~ishandle(fig) || ~isappdata(fig, 'record_state'); return; end
    state = getappdata(fig, 'record_state');
    items = get(state.h_instr_popup, 'String');
    if isempty(items); return; end
    val = get(state.h_instr_popup, 'Value');
    if iscell(items); sel = items{val}; else; sel = items(val,:); end
    if startsWith(sel, '(无建议代码)')
        return;
    end
    
    % 解析“CODE - NAME”
    parts = strsplit(sel, ' - ');
    instr_code = strtrim(parts{1});
    set(state.h_instr_code, 'String', instr_code);

    % 更新其它字段
    fn = ['code_' strrep(instr_code,'-','_')];
    if isfield(state.maintenance_db, fn)
        info = state.maintenance_db.(fn);
        if isfield(info,'fault_name'); set(state.h_name,'String',info.fault_name); end
        if isfield(info,'fault_type'); set(state.h_type,'String',info.fault_type); end
        if isfield(info,'severity');   set(state.h_severity,'String',info.severity); end
        if isfield(info,'priority');   set(state.h_priority,'String',num2str(info.priority)); end
    end
...
% 添加记录按钮
function onAddRecord(fig)
    if ~ishandle(fig) || ~isappdata(fig, 'record_state'); return; end
    state = getappdata(fig, 'record_state');

    % 读取表单
    rec = struct();
    rec.dataFaultCode       = strtrim(get(state.h_data_code, 'String'));
    rec.faultTime           = try_parse_time(get(state.h_time, 'String'));
    rec.instrumentFaultCode = strtrim(get(state.h_instr_code, 'String'));
    rec.faultName           = strtrim(get(state.h_name, 'String'));
    rec.faultType           = strtrim(get(state.h_type, 'String'));
    rec.severity            = strtrim(get(state.h_severity, 'String'));
    rec.priority            = str2double_safe(get(state.h_priority, 'String'));
    rec.operation           = strtrim(get(state.h_operation, 'String'));
    rec.duration            = strtrim(get(state.h_duration, 'String'));
    rec.tools               = strtrim(get(state.h_tools, 'String'));

    % 基础校验
    if isempty(rec.dataFaultCode) || isnat(rec.faultTime) || isempty(rec.operation)
        warndlg('请至少填写：数据故障代码、故障时间、维修操作。', '提示');
        return;
    end

    % 追加
    state.records(end+1) = rec; %#ok<AGROW>

    % 刷新表格
    update_records_table(state);

    % 保存回状态
    setappdata(fig, 'record_state', state);

    % 提示
    fprintf('【记录层】已添加一条维护记录：%s / %s\n', rec.dataFaultCode, datestr(rec.faultTime,'yyyy-mm-dd HH:MM:SS'));
end

% 导出Excel按钮
function onExportRecords(fig)
    if ~ishandle(fig) || ~isappdata(fig, 'record_state')
        warning('无法导出：未找到记录状态'); return; 
    end
    state = getappdata(fig, 'record_state');
    filename = '故障诊断维修记录.xlsx';

    % 生成数据矩阵
    header = {'数据故障代码','故障时间','仪表故障代码','故障描述/名称','故障类型','严重程度','优先级','维修操作','耗时','工具'};
    data = {};
    for i = 1:numel(state.records)
        r = state.records(i);
        data(end+1, :) = { ... %#ok<AGROW>
            r.dataFaultCode, ...
            datestr(r.faultTime, 'yyyy-mm-dd HH:MM:SS'), ...
            r.instrumentFaultCode, ...
            r.faultName, ...
            r.faultType, ...
            r.severity, ...
            r.priority, ...
    ...
    try
        if isempty(data)
            writecell([header; cell(1, numel(header))], filename);
        else
            writecell([header; data], filename);
        end
        fprintf('【记录层】已导出维修记录至: %s\n', filename);
    catch ME
        warning('导出维修记录失败：%s', ME.message);
    end
end

% 刷新表格
function update_records_table(state)
    tbl = state.h_records_table;
    data = {};
    for i = 1:numel(state.records)
        r = state.records(i);
        data(end+1, :) = { ... %#ok<AGROW>
...
% 将字符串转为datetime（失败返回NaT）
function dt = try_parse_time(s)
    try
...
% 数据故障代码到名称/类型（无建议时备用）
function [name, type] = data_fault_name_type(code)
    switch code
        case '001', name = '满量程输出';     type = '数据保持型/越限';
        case '002', name = '零位输出';       type = '数据保持型/越限';
        case '003', name = '数据缺失';       type = '数据传输中断型';
        case '004', name = '数据保持';       type = '数据保持型';
        case '005', name = '剧烈波动';       type = '数据波动型';
        case '006.01', name = '小幅正向偏移'; type = '数据偏差型';
        case '006.02', name = '小幅负向偏移'; type = '数据偏差型';
        case '006.03', name = '大幅正向偏移'; type = '数据偏差型';
        case '006.04', name = '大幅负向偏移'; type = '数据偏差型';
        case '006.05', name = '严重正向偏移'; type = '数据偏差型';
        case '006.06', name = '严重负向偏移'; type = '数据偏差型';
        otherwise, name = ''; type = '';
    end
end
