...
if config.enable_visualization
     fig = figure('Name', '实时氧分析仪监控系统 v4.0 - 含智能维修建议', ...
...
if config.save_log
     % 生成报告文件名
     report_filename = sprintf('O2_Monitor_Report_With_Maintenance_%s.txt', ...
     ...
     if stats.total_alarms > 0
         alarm_excel = sprintf('O2_Alarms_With_Maintenance_%s.xlsx', datestr(now, 'yyyymmdd_HHMMSS'));
...
% 保存最终图形
if config.enable_visualization
     fig_filename = sprintf('O2_Monitor_Final_With_Maintenance_%s.png', ...
                          datestr(now, 'yyyymmdd_HHMMSS'));
...
fprintf('\n【新增功能说明】\n');
fprintf('✓ 整合智能维修建议数据库\n');
fprintf('✓ 自动匹配故障代码与维修建议\n');
fprintf('✓ 提供详细的分步维修措施\n');
fprintf('✓ 显示维修优先级和预计耗时\n');
fprintf('✓ 记录完整的维修建议历史\n');
